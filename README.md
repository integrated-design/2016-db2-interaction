...
入力としてマウス、キーボード、サウンド（マイク）、カメラを用意しています。出力としては、様々なアニメーション操作方法を用意しています。
...
入力は、上記の出力と組み合わせて使います。例えば、「マウスをクリックしたら、インスタンスaを10フレーム目から再生すると同時に、インスタンスbを逆再生する」という組み合わせはこのようになります。

```
//マウスをクリックしたら
function onClick() {
    //インスタンスaを10フレーム目から再生する
    mcGotoAndPlay(a, 10);
...
これらは普通の入力と違って、システムによって自動的に実行される関数です。
